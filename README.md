...
>
> 🔒 **Going public later:** `config.yaml`, `work/`, and `output/` are already gitignored (never tracked). The one thing to handle before flipping public is `skill/kaushal-forge/examples/` — it ships **real sample data** as the gold-standard reference; anonymize it (or swap in a fictional sample) first.
...
- **Judgement** (write tailored content) → the AI, but **confined to filling rigid JSON schemas** with a worked example beside each (`skill/` or `prompts/`). A weak model pattern-matches instead of inventing, and **`engine/verify.py` is a hard gate** that catches slips.
...
- **In Claude Code:** invoke the **`kaushal-forge`** skill (see `skill/kaushal-forge/SKILL.md`) — it orchestrates P1→P6.
...
- `skill/kaushal-forge/` — the AI orchestrator skill (phases + schemas + rules + gold examples).
