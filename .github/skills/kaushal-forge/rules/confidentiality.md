...
8. **Human review & ownership.** Everything you produce is an **AI draft, not verified fact** — it represents a real person to recruiters. Never present it as final or move toward publishing on the operator's behalf. After each phase **STOP and have the human review it** (see SKILL.md), and remember the engine enforces this: the approved build (`build_pdfs.py --approved`) and all publishing refuse until the operator sets `reviewed: 1` in `work/review.yaml`. The operator owns accuracy; your job is an honest, traceable draft they can verify.
