...
5. Write `work/letters.json`, then **validate before rendering** (fix only the named fields, repeat until both pass):
   ```
   python engine/tools/validate.py work/letters.json    # schema conformance
   python engine/tools/rulecheck.py work/letters.json   # ASCII, mask leaks, HTML entities, GPA
   ```
   Then run `python engine/render_coverletters.py`.

## Output
`work/letters.json` → `output/CoverLetters/`. Both guardrails must pass before you finish.
